...
** Last update Sat Oct  9 22:46:43 2010 quentin pidoux
*/
...
void	letter(int x, int y, int nb_x,int nb_y)
{
  if ((nb_x == 0 && nb_y  == 0) 
     || (nb_x ==  0 && nb_y  == y - 1))
    {
      my_putstr("\033[1;34mA\033[1;00m");
    }
  if ((nb_x == x - 1 && nb_y  == 0) 
     || (nb_x == x - 1 && nb_y  == y - 1))
    {
      my_putstr("\033[1;35mC\033[1;00m");
    }
  if (nb_x != 0 && nb_x != x-1)
    {
      my_putstr("\033[1;32mB\033[1;00m");
    }
}

void	display_b(int x, int y, int nb_x, int nb_y)
{
